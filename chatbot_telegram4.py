...
def myanmar_tokenize(text):
    ...
    # Use regular expression to split by spaces and separate English words
...
def predict_class(sentence, model):
    ...
    # Perform Myanmar text segmentation first
    segmented_sentence = myanmar_tokenize(sentence)
    segmented_sentence = ' '.join(segmented_sentence)
    print(f"Segmented sentence: {segmented_sentence}")  # Debug
    
    # Normalize the segmented sentence
    normalized_sentence = normalize_text(segmented_sentence, synonyms)
    print(f"Normalized sentence: {normalized_sentence}")  # Debug
    
    # Convert the normalized sentence into a bag of words representation
...
def main() -> None:
    ...
    # Set up logging
    logging.basicConfig(format='%(asctime)s - %(name)s - %(level.levelname)s - %(message)s', level=logging.INFO)
